<h1 align="center">Hi 👋, I'm Alvin Onyiego Ondieki</h1>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=300&center=true&vCenter=true&width=600&lines=Data+Scientist;AI+Engineer;Software+Engineer;IT+Specialist;Flutter+%26+Backend+Developer" />
</p>

<p align="center">
  <b>I build data-driven systems, AI solutions, scalable software, and cloud-based IT infrastructure.</b><br>
  Solving real-world problems in healthcare, research, and enterprise.
</p>

<p align="center">
  <a href="https://alvin-ondieki-portfolio.abrdns.com/" target="_blank">Portfolio</a> •
  <a href="https://x.com/alvin_ondieki" target="_blank">Twitter/X</a> •
  <a href="https://www.linkedin.com/in/alvin-ondieki" target="_blank">LinkedIn</a> •
  <a href="https://play.google.com/store/apps/details?id=com.xoma.bookstores" target="_blank">XOMA App</a> •
  <a href="https://play.google.com/store/apps/details?id=com.clsluxurycars.app" target="_blank">CLC Cars App</a>
</p>

---

## 👨‍💻 About Me
I am a **Data Scientist, Software & Systems Developer, and IT Specialist** with expertise in **Machine Learning, Deep Learning, NLP, AI, predictive analytics, cross-platform mobile apps, and full-stack web development**.  

I deliver **end-to-end solutions** — from data collection, ETL pipelines, predictive modeling, dashboards, mobile/web apps, APIs, IoT integration, cloud deployment, to secure IT systems.  

💡 Passionate about building **reliable, scalable, and impactful technology** to optimize workflows, enhance decision-making, and support enterprise & healthcare operations.

---

## 🛠 Technical Skills

**Programming & Development:**  
Python, Java, C++, C, C#, Dart, JavaScript, PHP  

**Frameworks & Mobile:**  
Flutter, React.js, React Native, Django, Flask, Node.js, Spring Boot, Angular  

**Data Science & AI:**  
Machine Learning, Deep Learning, NLP, Transformers, Predictive Modeling, Time-Series Forecasting, ETL, MLOps, Model Deployment  

**Visualization & Analysis:**  
Pandas, NumPy, Matplotlib, Seaborn, Plotly, Power BI, Tableau  

**Databases & Cloud:**  
PostgreSQL, MySQL, MongoDB, SQLite, Firebase, Supabase, AWS, Azure, GCP, Apache Spark, Hadoop  

**Systems & Tools:**  
Docker, Kubernetes, Linux, VMware, Arduino, Raspberry Pi, REDCap, ODK, DF-Net  

**Soft Skills:** Critical Thinking, Problem Solving, Teamwork, Adaptability, Time Management, Continuous Learning  

---

## 🚀 Featured Projects

### 📱 Mobile & Cross-Platform Apps
- **[XOMA App](https://play.google.com/store/apps/details?id=com.xoma.bookstores)** – Flutter/Dart + Supabase, scalable, smooth UI/UX  
- **[CLC Cars App](https://play.google.com/store/apps/details?id=com.clsluxurycars.app)** – Flutter + REST APIs + Supabase  
- Android Final IT Project – Java-based app with modern UI/UX  

### 🤖 AI & Data Science
- Early Alzheimer’s Detection – CNN-Transformer hybrid (95% accuracy on 120k+ scans)  
- Waterborne Disease Forecasting – Time-series ML models  
- ETL Pipelines & Predictive Analytics – Cleaned, modeled, visualized data  

### 🌐 Full-Stack & Web Systems
- **PowerChins Construction** – Django web app with PostgreSQL, deployed on GCP, project management & site monitoring system  
- AI-powered web apps using Django, React.js, Node.js  
- RESTful API development & integration  
- Cloud deployment & secure database optimization  

### 🌐 IoT & Embedded
- Arduino & Raspberry Pi sensor integration  
- MQTT protocols for real-time data communication  

---

## 🏆 Hackathons & Competitions
- AfriCDSA Hackathon (2025) – Waterborne Disease Forecasting  
- DeKUT AI Challenge (2025) – Early Alzheimer’s Detection  
- Kaggle Projects – Supervised/Unsupervised ML, NLP, Computer Vision, Time-Series  

---

## 🎓 Education
- **BSc Information Technology** – Dedan Kimathi University of Technology (Upper Division, 2024)  
- **Data Science & AI** – SKIES Rapid Tech Skills Program, DeKUT (2025)  

---

## 🏅 Certifications
- AWS AI Practitioner (2025)  
- Data Science & AI (DeKUT, 2025)  
- Cybersecurity (Cisco, 2023)  
- Linux Essentials (Cisco, 2023)  
- Networking (Cisco, 2022)  

---

## 📊 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=alvinplp2024&show_icons=true&count_private=true&theme=radical" alt="Alvin's GitHub Stats" />  
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=alvinplp2024&theme=radical" alt="Alvin's GitHub Streak" />
  <br>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=alvinplp2024&layout=compact&theme=radical" alt="Alvin's Top Languages" />
</p>

---

## 📫 Connect With Me
- 📧 Email: **alvinondieki5@gmail.com**  
- 🌐 Portfolio: [Portfolio](https://alvin-ondieki-portfolio.abrdns.com/)  
- 🐦 Twitter/X: [Twitter](https://x.com/alvin_ondieki)  
- 🔗 LinkedIn: [LinkedIn](https://linkedin.com/in/alvin-ondieki)  
- 📸 Instagram: [Instagram](https://www.instagram.com/data_scientist045/)  

---

⭐ *Open to Data Science, AI Engineer, Software Engineer, IT Specialist, and Full-Stack Developer roles*
